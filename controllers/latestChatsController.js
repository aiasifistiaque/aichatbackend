...
const updateLatestChats = async doc => {
	...
	if (findSender) {
		...
		io.of('/users')
			.to(`activechats${sender}`)
			.emit('updateactivechats', findSender);
		findSender.save();
	} else {
		...
		io.of('/users')
			.to(`activechats${sender}`)
			.emit('addactivechats', postSender);
	...
	if (findReceiver) {
		...
		io.of('/users')
			.to(`activechats${receiver}`)
			.emit('updateactivechats', findReceiver);
		...
		io.of('/users')
			.to(`activechats${receiver}`)
			.emit('addactivechats', postReceiver);
